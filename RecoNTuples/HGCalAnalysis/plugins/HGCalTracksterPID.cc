...
class HGCalTracksterPID : public edm::one::EDAnalyzer<edm::one::WatchRuns, edm::one::SharedResources> {
 public:
  //
  ...
  HGCalTracksterPID();
  explicit HGCalTracksterPID(const edm::ParameterSet &);
  ~HGCalTracksterPID();
...
HGCalTracksterPID::HGCalTracksterPID() { ; }

HGCalTracksterPID::HGCalTracksterPID(const edm::ParameterSet &iConfig)
    : readCaloParticles_(iConfig.getParameter<bool>("readCaloParticles")),
      ...
      storeCPMatching_(iConfig.getParameter<bool>("storeCPMatching")),
      ...
      forPIDOnly_(iConfig.getParameter<bool>("forPIDOnly")),
  ...
  if (storeCPMatching_)
  {
    ...
    t_->Branch("cluster2d_cpEnergy", &cluster2d_cpEnergy_);

    t_->Branch("cluster2d_best_cpId", &cluster2d_best_cpId_);
    t_->Branch("cluster2d_best_cpPdg", &cluster2d_best_cpPdg_);
    t_->Branch("cluster2d_best_cpEnergy", &cluster2d_best_cpEnergy_);
...
// member functions
//
void HGCalTracksterPID::clearVariables() {
  ev_run_ = 0;
  ev_lumi_ = 0;
  ev_event_ = 0;
  vtx_x_ = 0;
  ...
  cluster2d_cpEnergy_.clear();

  cluster2d_best_cpId_.clear();
  cluster2d_best_cpPdg_.clear();
  cluster2d_best_cpEnergy_.clear();
...
void HGCalTracksterPID::analyze(const edm::Event &iEvent, const edm::EventSetup &iSetup) {
  using namespace edm;
  ...
  HGCal_helpers_pid::simpleTrackPropagator toHGCalPropagator(aField_);
  ...
  for (unsigned int i = 0; i < npart; ++i) {
    ...
    {
      HGCal_helpers_pid::coordinates propcoords;
      ...
      HGCal_helpers_pid::simpleTrackPropagator indiv_particleProp(aField_);
    ...
    HGCal_helpers_pid::coordinates hitsHGCal;
...
void HGCalTracksterPID::beginRun(edm::Run const &iEvent, edm::EventSetup const &es) {
...
void HGCalTracksterPID::endRun(edm::Run const &iEvent, edm::EventSetup const &) {}

void HGCalTracksterPID::beginJob() { ; }

// ------------ method called once each job just after ending the event loop
// ------------
void HGCalTracksterPID::endJob() {}

// ------------ method to be called once
// --------------------------------------------------

void HGCalTracksterPID::retrieveLayerPositions(const edm::EventSetup &es, unsigned layers) {
  recHitTools_.getEventSetup(es);
...
void HGCalTracksterPID::fillRecHit(const DetId &detid, const float &fraction, const unsigned int &layer,
...
void HGCalTracksterPID::fillDescriptions(edm::ConfigurationDescriptions &descriptions) {
...
/*
   Surface::RotationType HGCalTracksterPID::rotation( const GlobalVector& zDir)
   const
   {
...
DEFINE_FWK_MODULE(HGCalTracksterPID);